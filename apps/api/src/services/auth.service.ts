...
export const authService = {
  async sendOtp(rawPhone: string): Promise<ServiceResult<{ sent: true }>> {
    ...
    if (!phone) {
      return { ok: false, status: 400, code: "INVALID_PHONE", message: "Enter a valid phone number in E.164 format." };
    }

    const recent = await countRecentSends(phone);
    if (isSendRateLimited(recent)) {
      return { ok: false, status: 429, code: "RATE_LIMITED", message: "Too many OTP requests. Try again later." };
    }
  ...
  async verifyOtp(
    ...
    ServiceResult<{ accessToken: string; refreshToken: string; user: { id: string; name: string; phone: string; role: Role } }>
  > {
    const phone = normalizePhone(rawPhone);
    if (!phone) {
      return { ok: false, status: 400, code: "INVALID_PHONE", message: "Enter a valid phone number in E.164 format." };
    ...
    if (!result.ok) {
      return {
        ...
        ...(result.attemptsLeft !== undefined ? { details: { attemptsLeft: result.attemptsLeft } } : {}),
      };
    }
  ...
  async refresh(token: string | undefined): Promise<ServiceResult<{ accessToken: string; refreshToken: string }>> {
    if (!token) {
    ...
    if (!verified.ok) {
      const message = verified.code === "TOKEN_EXPIRED" ? "Refresh token expired." : "Invalid refresh token.";
  ...
  async getMe(userId: string): Promise<ServiceResult<{ id: string; name: string; phone: string; email: string | null; role: Role; status: string }>> {
    const user = await userRepository.findMeById(userId);
    if (!user) {
      return { ok: false, status: 401, code: "UNAUTHORIZED", message: "User not found." };
    }
    return { ok: true, data: user as { id: string; name: string; phone: string; email: string | null; role: Role; status: string } };
  },
};
