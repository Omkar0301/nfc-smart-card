...
*As a developer,* I need the database schema to accurately represent every lifecycle state defined in the PRD, enforce relational integrity between Users, Profiles, and CardAssignments, and have indexes on high-traffic lookup paths — so that all subsequent features can be built on a correct, stable foundation.
